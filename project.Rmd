...
library(tidyverse)
cpmerge <- cpmerge %>% 
  filter(Country %in% c("United States", "Congo", "Brazil", "Australia", "Norway", "China"), na.rm = TRUE) %>%
  group_by(Country) %>% 
...
ggplot(cpmerge, aes(avgpop, avgco2, col = Country)) +
  scale_color_manual(values = c("Congo" = "skyblue", "United States" = "pink", "Brazil" = "darkgreen", "Australia" = "maroon", "Norway" = "orange", "China" = "yellow")) +
  geom_point(size = 3) +
...
co2l %>% 
  ...
  filter(Country %in% c("Congo", "United States", "Brazil", "Australia", "Norway", "China")) %>% 
  ggplot(aes(Year, `CO2 Production`, fill = Country)) +
  scale_fill_manual(values = c("Congo" = "skyblue", "United States" = "pink", "Brazil" = "darkgreen", "Australia" = "maroon", "Norway" = "orange", "China" = "yellow")) +
...
hcmerge %>% 
  filter(Year == "2021") %>% 
  filter(Country %in% c("United States", "Congo", "Brazil", "Australia", "Norway", "China")) %>% 
