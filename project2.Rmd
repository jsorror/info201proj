...
```{r}
data <- read_delim("Merged_data.txt")
```
...
mapping <- function(input_year){
  data_map <- world %>% 
    ...
    merge(data %>% filter(Year == input_year), by = "Country", all.x = TRUE) %>% 
    group_by(group) %>% 
    arrange(order)
...
merge %>% 
  ...
  scale_fill_manual(values = c("Nigeria" = "skyblue", "United States" = "pink", "Brazil" = "darkgreen", "Australia" = "maroon", "France" = "orange", "China" = "yellow")) +
  geom_col(position = "dodge", col = "black") +
  labs(title = "CO2 Production in 1990 vs. 2021")
```
...
```{r}
mergesum <- merge %>%
  filter(Country %in% c("United States", "Nigeria", "Brazil", "Australia", "France", "China"), na.rm = TRUE) %>%
  group_by(Country) %>%
...
merge %>% 
  filter(Year == "2021") %>% 
  filter(Country %in% c("United States", "Nigeria", "Brazil", "Australia", "France", "China")) %>% 
