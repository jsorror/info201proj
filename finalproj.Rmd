...
2019. This is to see how CO2 has changed in the past 30 years and how it is different for each country. Then we compare the Human 
...
Finally, we compare HDI with the CO2 production in 2019 since cement production and fossil fuel burning are strong contributors of the CO2 
...
have 4,980 observations in our dataset. The relevant variables are “Country”, which is the country’s name, “Country Code”, containing the 
three-letter country code, “Year”, with years spanning from 1990 to 2019, “CO2 Production”, depicting the total carbon emissions in a 
country in a certain year, “Population”, which states the total country population in a given year, “HDI”, which depicts the HDI for a 
country in a given year, “CO2Index”, which is a categorical variable that gives a certain index of a country’s CO2 level depending on where 
it falls in our caliber (3,569,073 kt and below = “Low”, 3,569,074 kt to 7,138,146 = “Middle”, and 7,138,147 kt and above = “High”), 
“AvgCO2”, which is a numerical variable that gives the calculated average CO2 production for a certain country across all years, and 
finally, “AvgPop”, which is a numerical variable that gives the calculated average population for a certain country across all years. 
Additionally, we created a summarized dataset called “mergesum” containing the average HDI levels for each country and the average 
population for each country. There are a few missing values in the key variables for our dataset. The “Population” variable has 11 missing 
values, and the “HDI” variable has 519 missing values. Although we had a few missing values, which may have deterred the quality of our 
dataset, it didn’t hinder our ability to plot the relationships we wanted to. Additionally, our idea of creating the “AvgPop” variable in 
order to plot the average values of this data allowed the missing values to decrease significantly, which made it easier for us to plot the 
relationships for the countries we chose.
...
take only one variable (the yearly measures of co2, hdi, and population) for the merged final product in order to be as clean as possible 
for our final data manipulation to create our plots. We also appreciated the very raw variables we chose because they were easier to 
manipulate into the secondary (categorical/numerical) variables required for the project.
...
country code variable that we also have access to in all datasets will be the common variables that allows us to merge it all into one. For 
all of the datasets we need to make unified the naming conventions for the country code variables (“Country Code”). Individually we also 
want to make simple the naming convention for all column names that represent a single year's worth of data. Co2 for example has a prefix 
“co2_prod_” before the year. This is done for Co2 and HDI using sub() and assignment to names(<data>) to replace their respective prefixes 
with empty (“”) strings before any pivoting of the datasets. In the case of the population dataset we also use rename() to rename the 
variable “Country Name” to be “Country” for simplicity. 	
...
only focused on the yearly information recorded for each country in its respective subject. For hdi this is a decimal value index every 
year so we create a second version of this dataset in longer form, then after that we create a third that strips away extraneous variables. 
These extraneous variables are hdi code, region, and rank 2021. This makes the data cleaner as information like region will be the same for 
each country no matter the year and just takes up space in this longer format without additional information provided. This limits us 
however as we are now unable to make plots based on region for example to group countries, something we decided is not important to the 
plots we want to make. The same is done for the human development index (hdi code, region, and rank in 2021) and we strip the Country Name 
variable from both the HDI and Co2 datasets so that we just use the names from the population dataset. There is some difference in country 
names between the sets so to avoid losing data we just use the names from one.
...
final one. In doing so we only keep data that has both the same country code and a year of data on file. This allows us to keep the most 
data possible from our different sets as the country codes are the same between each of the datasets. Though they each have different 
ranges of time that they cover. Thus where co2 has data up until 2019 as opposed to the other two that go up until 2021. We end up losing 
the data for those non-similar years but this doesn’t affect us too much because we wouldn’t want to have information we can’t compare with 
anything anyways. 

With this final dataset we make some final decisions. We create a co2 index variable (categorical) and an average population variable 
(numerical) to additionally measure our data. We do this ignoring rows with N/A values in the respective original variable. We also make 
sure to mutate() our year variable to be numeric as opposed to a string.

As for our sample chosen, when making the bar chart we decided to focus only on some specific countries. This is for simplicity of display 
and audience understanding as we have a lot of countries that could be represented. This bar chart gives a snapshot with one country from 
each continent to get a general idea of how the leading countries in each region compare to each other as Australia, Brazil, China, Russia, 
the United States, and Nigeria all have the highest populations in their respective countries. All the other plots we created use the 
entirety of our data excluding any n/a values which there were few of. 

We did not end up using many complex statistical methods, though our scatterplots did have an aspect of linear regression. We didn’t do any 
solving for specific correlation coefficients, but we did add trend lines to our graphs. The first of which for our HDI vs Population 
scatterplot we used a linear model to show a downward trend. With this, we see visually that there is a wide spread away from the line and 
thus the relationship is weak with many possible outliers. For our second graph we allowed rstudio to generate its own trend line that is 
non-linear and here we see that for the left section of the data where most of our points are there is a clear positive trend until some 
large outliers change the trajectory for the line. 
...
production include China, the United States, and India. Similarly, for the population map, some of the countries with the highest 
population are China, the United States, and India.
